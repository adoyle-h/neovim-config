...
-- @value new_termopen_previewer = term_previewer.new_termopen_previewer
--
...
-- --- This will respect user configuration and will use `buffer_previewers` in
-- --- case it's configured that way.
-- @value vimgrep = term_previewer.vimgrep
--
-- --- Provides a `termopen_previewer` which has the ability to display files at
...
-- --- case it's configured that way.
-- @value qflist = term_previewer.qflist
--
...
-- @value new_buffer_previewer = buffer_previewer.new_buffer_previewer
--
...
-- @value buffer_previewer_maker = buffer_previewer.file_maker
--
...
-- --- case it's configured that way.
-- @value vim_buffer_cat = buffer_previewer.cat
--
...
-- --- This will respect user configuration and will use `termopen_previewer` in
-- --- case it's configured that way.
-- @value vim_buffer_vimgrep = buffer_previewer.vimgrep
--
...
-- --- This will respect user configuration and will use `termopen_previewer` in
-- --- case it's configured that way.
-- @value vim_buffer_qflist = buffer_previewer.qflist
--
-- --- A previewer that shows a log of a branch as graph
-- @value git_branch_log = buffer_previewer.git_branch_log
--
-- --- A previewer that shows a diff of a stash
-- @value git_stash_diff = buffer_previewer.git_stash_diff
--
...
-- @value git_commit_diff_to_parent = buffer_previewer.git_commit_diff_to_parent
--
-- --- A previewer that shows a diff of a commit to head.<br>
-- --- The run command is `git --no-pager diff --cached $SHA -- $CURRENT_FILE`
-- ---
-- --- The current file part is optional. So is only uses it with bcommits.
-- @value git_commit_diff_to_head = buffer_previewer.git_commit_diff_to_head
--
...
-- @value git_commit_diff_as_was = buffer_previewer.git_commit_diff_as_was
--
-- --- A previewer that shows the commit message of a diff.<br>
-- --- The run command is `git --no-pager log -n 1 $SHA`
-- @value git_commit_message = buffer_previewer.git_commit_message
--
-- --- A previewer that shows the current diff of a file. Used in git_status.<br>
-- --- The run command is `git --no-pager diff $FILE`
-- @value git_file_diff = buffer_previewer.git_file_diff
--
-- @value ctags = buffer_previewer.ctags
-- @value builtin = buffer_previewer.builtin
-- @value help = buffer_previewer.help
-- @value man = buffer_previewer.man
-- @value autocommands = buffer_previewer.autocommands
-- @value highlights = buffer_previewer.highlights
-- @value pickers = buffer_previewer.pickers
--
...
-- @value display_content = buffer_previewer.display_content

-- @param opts {table}
-- @param opts.setup {function(self)} Will be called the first time preview will be called.
-- @param opts.teardown {function(self)} Will be called on cleanup.
-- @param opts.preview_fn {function(self, entry, status)}
--			  Will be called each time a new entry was selected.
-- @param opts.title {function(self)} Will return the static title of the previewer.
-- @param opts.dynamic_title {function(self, entry)}
--        Will return the dynamic title of the previewer.
--        Will only be called when config value dynamic_preview_title is true.
-- @param opts.send_input {function(self, input)}
--				This is meant for `termopen_previewer` and it can be used to send
--				input to the terminal application, like less.
-- @param opts.scroll_fn {function(self, direction)}
--				Used to make scrolling work.
function P.get(name, opts)
	return previewers[name].new(opts or {})
end
